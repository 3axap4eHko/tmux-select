...
impl ControlClient {
    ...
    pub fn enumerate(&mut self, session_id: &str) -> Result<Vec<Pane>> {
        let body = self.command(&format!(
            "list-panes -s -t '{session_id}' -F '{PANE_FORMAT}{RECORD_SEP}'"
        ))?;
        parse_panes(&body)
    }
...
fn parse_panes(body: &str) -> Result<Vec<Pane>> {
    body.split(RECORD_SEP)
...
";
    ...
    #[test]
    fn record_separator_keeps_a_newline_in_the_path_intact() {
        let body = format!(
            "%2|@0|12|1|3|123|claude|/home/me/a\nb{RECORD_SEP}\n%3|@0|12|0|4|124|bash|/tmp{RECORD_SEP}"
        );
        let panes = parse_panes(&body).unwrap();
        assert_eq!(panes.len(), 2);
        assert_eq!(panes[0].current_path, "/home/me/a\nb");
        assert_eq!(panes[1].current_path, "/tmp");
    }
}
